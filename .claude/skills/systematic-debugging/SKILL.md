...
- `ng serve` terminal output (compilation errors)
- Network tab in browser dev tools (API responses, status codes)
...
- **Where** does it fail? (Component? Service? API call? Route guard?)
...
| 401/403 on API calls | JWT token expired or interceptor not attaching token |
| CORS errors | API CORS config doesn't match frontend origin |
| API calls returning unexpected data | API contract changed or wrong endpoint URL |
